...
const { getTwitterHandle, parseTwitterHandle, getVtuberData, getYouTubeChannelName, youTubeRegex } = require('./socialMedia');
...
class Uploader {
    ...
    async generateVideoDescription (channel, socialMediaLinks) {
        ...
        // also, if the href is a yt link, keep track of it for later
        ...
        if (ytChannels.length > 0) {
            ...
            for (const href of ytChannels) {
                const ytChannelName = await getYouTubeChannelName(href);
