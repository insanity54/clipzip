...
function configureEditlyLayer (clipDuration, clipInfo, fileName) {
  ...
  console.log(`the duration is ${clipDuration}`);
  return {
    layers: [
      {
        type: 'video',
...
async function combineClips (argv) {
  ...
  for await (clipInfo of manifest) {
    ...
    console.log(`clipFileName:${clipFileName}, duration:${duration}`);
  ...
  const combinedFilename = path.join(rootOutputDir, `${DateTime.now().toMillis()}.mp4`);
  let videoParams = await getEditlySpec(fileNames, fps, width, height, clipsSpecification, combinedFilename);
  await editly(videoParams);
  console.log(combinedFilename);
  const p = path.parse(combinedFilename);
  const normalizedFilename = path.resolve(rootOutputDir, `${p.name}_norm${p.ext}`);
  await NormalizeVolume(combinedFilename, normalizedFilename);
